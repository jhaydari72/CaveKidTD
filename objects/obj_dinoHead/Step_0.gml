	...
	if (variable_instance_exists(collisionId, "purpose")) {
		...
		if (instVar == id) {
			if (temptower == 1) {
				...
				tower = instance_create_layer(x,y,"Instances",obj_catapult)
			}
			else if (temptower == 3) {
				tower = instance_create_layer(x,y,"Instances",obj_anklyo_tower)
			}
			else if (temptower == 4){
				tower = instance_create_layer(x,y,"Instances",obj_spit_tower)
			}
